...
const CreatePost = (props) => {
    ...
    const uploadPost = async () => {
        ...
        try{
            let messContent = '';
            const doc = await firebase.firestore().collection('posts').doc();
            ...
            if(videoUrl != null)
            {
                messContent = ' video mới'
                obj = {...obj, ...{video: videoUrl}}
            }
            if(messContent=='') messContent=' bài viết mới'
            console.log(obj)
            doc.set({...obj})
                .then(res=> {
                ...
                .catch(error => {
                    ...
                    ToastAndroid.show("Đăng bài thất bại " +error.message, ToastAndroid.SHORT)
                    setLoading(false)
                })
        }
        catch(err)
        ...
        <View style={styles.container}>
            <View style={styles.topBar}>
                ...
                <Pressable  onPress={uploadPost} disabled={!canPost}>
                    <Text style={[styles.postPress, canPost && { color: 'blue', fontWeight: '500' }]}>POST</Text>
                </Pressable>
            </View>
            ...
            <ScrollView>
                ...
                />
                <MyVideo uri={videoPicked?.uri} />
                <AddedImageView data={imagePicked} style={{alignSelf: 'flex-end'}} />
            </ScrollView>
